                ...
                <Link href={`/blog/${post.id}`}>
                  <div className="bg-card border rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all hover:-translate-y-0.5 cursor-pointer flex flex-col md:flex-row">
                    <div className="md:w-64 flex-shrink-0">
                      {post.imageUrl ? (
                        <img src={post.imageUrl} alt={post.title} className="w-full h-48 md:h-full object-cover" onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} />
                      ) : (
                    ...
                    <div className="p-6 flex-1">
                      <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground mb-3">
                        <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{new Date(post.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}</span>
                      ...
                      <p className="text-muted-foreground text-sm leading-relaxed line-clamp-2 mb-2">{post.contentHindi.substring(0, 200)}...</p>
                      <p className="text-muted-foreground text-xs leading-relaxed line-clamp-2">{post.content.substring(0, 150)}...</p>
                      <div className="mt-4 text-primary text-sm font-semibold flex items-center gap-1">पूरा पढ़ें / Read More <ChevronRight className="w-4 h-4" /></div>
                    </div>
                  </div>
                </Link>
              </motion.div>
